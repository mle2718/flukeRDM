...
coastwide <- files %>%
  ...
  dplyr::filter(!model %in% c("SQ", "SQ1", "SQ2")) %>% 
  dplyr::bind_rows(SQ)
...
harv <- coastwide2 %>% #all_data %>%
 ...
 tab<- harv %>% 
      ...
      # ) %>% 
      dplyr::rename( `Run Name` = model,
                    `BSB Median harvest weight`= bsb, `Scup Median harvest weight`= scup, `SF Median harest weight` = sf)
