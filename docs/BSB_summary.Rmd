...
harv <- dat %>% 
 ...
 tab<- harv %>% 
      ...
      dplyr::rename( `Policy Name` = model,
                    `BSB median harvest pounds`= bsb, `Scup median harvest pounds`= scup, `SF median harvest pounds` = sf)
...
DT::datatable(
  ...
  options = list(
    ...
    autoWidth = TRUE)) %>% 
  DT::formatRound(
    columns = c("BSB median harvest pounds",
                "Scup median harvest pounds",
                "SF median harvest pounds"),
    digits = 0,mark = ",")
```
...
discs <- dat %>% 
 ...
 tab<- discs %>% 
      ...
      dplyr::select( -metric) %>%
      dplyr::rename( `Policy Name` = model,
                    `BSB median discard pounds`= bsb, `Scup median discard pounds`= scup, `SF median discard pounds` = sf)
...
DT::datatable(
  ...
  DT::formatRound(
    columns = c("BSB median discard pounds",
                "Scup median discard pounds",
                "SF median discard pounds"),
    digits = 0,mark = ",")
```
