...
if(input$SF_CT_input_type == "All Modes Combined"){
  directed_trips<- directed_trips %>%
    dplyr::mutate(#Summer Flounder
      ...
      fluke_min_y2=dplyr::case_when(day_i >= lubridate::yday(SFct_seas1_op) & day_i <= lubridate::yday(SFct_seas1_cl) ~ as.numeric(SFct_1_len), TRUE ~ 100), 
      ...
      fluke_min_y2=dplyr::case_when(day_i >= lubridate::yday(SFct_seas2_op) & day_i <= lubridate::yday(SFct_seas2_cl) ~ as.numeric(SFct_2_len), TRUE ~ fluke_min_y2), 
      ...
      fluke_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(SFctFH_seas3_op) & day_i <= lubridate::yday(SFctFH_seas3_cl) ~ as.numeric(SFctFH_3_len), TRUE ~ fluke_min_y2),
      fluke_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(SFctPR_seas3_op) & day_i <= lubridate::yday(SFctPR_seas3_cl) ~ as.numeric(SFctPR_3_len), TRUE ~ fluke_min_y2),
      fluke_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(SFctSH_seas3_op) & day_i <= lubridate::yday(SFctSH_seas3_cl) ~ as.numeric(SFctSH_3_len), TRUE ~ fluke_min_y2))
} else {
  directed_trips<- directed_trips %>%
    dplyr::mutate(
      ...
      fluke_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(SFctFH_seas1_op) & day_i <= lubridate::yday(SFctFH_seas1_cl) ~ as.numeric(SFctFH_1_len), TRUE ~ 100), 
      fluke_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(SFctPR_seas1_op) & day_i <= lubridate::yday(SFctPR_seas1_cl) ~ as.numeric(SFctPR_1_len), TRUE ~ fluke_min_y2), 
      fluke_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(SFctSH_seas1_op) & day_i <= lubridate::yday(SFctSH_seas1_cl) ~ as.numeric(SFctSH_1_len), TRUE ~ fluke_min_y2),
      
      fluke_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(SFctFH_seas2_op) & day_i <= lubridate::yday(SFctFH_seas2_cl) ~ as.numeric(SFctFH_2_len), TRUE ~ fluke_min_y2),
      fluke_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(SFctPR_seas2_op) & day_i <= lubridate::yday(SFctPR_seas2_cl) ~ as.numeric(SFctPR_2_len), TRUE ~ fluke_min_y2),
      fluke_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(SFctSH_seas2_op) & day_i <= lubridate::yday(SFctSH_seas2_cl) ~ as.numeric(SFctSH_2_len), TRUE ~ fluke_min_y2), 
...
directed_trips<- directed_trips %>%
  dplyr::mutate(# Black Sea Bass Bag Limit by Mode
    ...
    bsb_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(BSBctFH_seas1_op) & day_i <= lubridate::yday(BSBctFH_seas1_cl) ~ as.numeric(BSBctFH_1_len), TRUE ~ 100), 
    bsb_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(BSBctPR_seas1_op) & day_i <= lubridate::yday(BSBctPR_seas1_cl) ~ as.numeric(BSBctPR_1_len), TRUE ~ bsb_min_y2), 
    bsb_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(BSBctSH_seas1_op) & day_i <= lubridate::yday(BSBctSH_seas1_cl) ~ as.numeric(BSBctSH_1_len), TRUE ~ bsb_min_y2),
    bsb_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(BSBctFH_seas2_op) & day_i <= lubridate::yday(BSBctFH_seas2_cl) ~ as.numeric(BSBctFH_2_len), TRUE ~ bsb_min_y2), 
    bsb_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(BSBctPR_seas2_op) & day_i <= lubridate::yday(BSBctPR_seas2_cl) ~ as.numeric(BSBctPR_2_len), TRUE ~ bsb_min_y2), 
    bsb_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(BSBctSH_seas2_op) & day_i <= lubridate::yday(BSBctSH_seas2_cl) ~ as.numeric(BSBctSH_2_len), TRUE ~ bsb_min_y2), 
    bsb_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(BSBctFH_seas3_op) & day_i <= lubridate::yday(BSBctFH_seas3_cl) ~ as.numeric(BSBctFH_3_len), TRUE ~ bsb_min_y2), 
    bsb_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(BSBctPR_seas3_op) & day_i <= lubridate::yday(BSBctPR_seas3_cl) ~ as.numeric(BSBctPR_3_len), TRUE ~ bsb_min_y2), 
    bsb_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(BSBctSH_seas3_op) & day_i <= lubridate::yday(BSBctSH_seas3_cl) ~ as.numeric(BSBctSH_3_len), TRUE ~ bsb_min_y2),
    ...
    scup_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(SCUPctFH_seas1_op) & day_i <= lubridate::yday(SCUPctFH_seas1_cl) ~ as.numeric(SCUPctFH_1_len), TRUE ~ 100),
    ...
    scup_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(SCUPctFH_seas2_op) & day_i <= lubridate::yday(SCUPctFH_seas2_cl) ~ as.numeric(SCUPctFH_2_len), TRUE ~ scup_min_y2),
    ...
    scup_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(SCUPctFH_seas3_op) & day_i <= lubridate::yday(SCUPctFH_seas3_cl) ~ as.numeric(SCUPctFH_3_len), TRUE ~ scup_min_y2),
    ...
    scup_min_y2=dplyr::case_when(mode == "fh" & day_i >= lubridate::yday(SCUPctFH_seas4_op) & day_i <= lubridate::yday(SCUPctFH_seas4_cl) ~ as.numeric(SCUPctFH_4_len), TRUE ~ scup_min_y2),
    ...
    scup_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(SCUPctPR_seas1_op) & day_i <= lubridate::yday(SCUPctPR_seas1_cl) ~ as.numeric(SCUPctPR_1_len), TRUE ~ scup_min_y2),
    ...
    scup_min_y2=dplyr::case_when(mode == "pr" & day_i >= lubridate::yday(SCUPctPR_seas2_op) & day_i <= lubridate::yday(SCUPctPR_seas2_cl) ~ as.numeric(SCUPctPR_2_len), TRUE ~ scup_min_y2),
    ...
    scup_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(SCUPctSH_seas1_op) & day_i <= lubridate::yday(SCUPctSH_seas1_cl) ~ as.numeric(SCUPctSH_1_len), TRUE ~ scup_min_y2),
    ...
    scup_min_y2=dplyr::case_when(mode == "sh" & day_i >= lubridate::yday(SCUPctSH_seas2_op) & day_i <= lubridate::yday(SCUPctSH_seas2_cl) ~ as.numeric(SCUPctSH_2_len), TRUE ~ scup_min_y2))
